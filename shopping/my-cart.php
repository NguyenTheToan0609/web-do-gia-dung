...
// Nếu người dùng nhấn nút "submit" để cập nhật giỏ hàng
if(isset($_POST['submit'])){
	if(!empty($_SESSION['cart'])){
		foreach($_POST['quantity'] as $key => $val){
			if($val == 0){
				unset($_SESSION['cart'][$key]); // Xóa sản phẩm nếu số lượng là 0
			}else{
				$_SESSION['cart'][$key]['quantity'] = $val; // Cập nhật số lượng sản phẩm
			}
		}
...
// Mã để xóa sản phẩm khỏi giỏ hàng
...
// Mã để thêm sản phẩm vào bảng đơn hàng
if(isset($_POST['ordersubmit'])) {
	if(strlen($_SESSION['login']) == 0){   
		...
		foreach($value as $qty => $val34){
			...
			header('location:payment-method.php'); // Chuyển hướng đến trang phương thức thanh toán
		}
	}
}

// Mã để cập nhật địa chỉ thanh toán
...
// Mã để cập nhật địa chỉ giao hàng
...
<div class="form-group">
<label class="info-title" for="Địa chỉ thanh toán">Địa chỉ <span>*</span></label>
					  ...
					  <div class="form-group">
    <label class="info-title" for="Billing State ">Tỉnh/Thành phố thanh toán <span>*</span></label>
...
<div class="form-group">
    <label class="info-title" for="Billing Pincode">Mã bưu điện thanh toán <span>*</span></label>
...
<div class="form-group">
<label class="info-title" for="Địa chỉ giao hàng">Địa chỉ giao hàng<span>*</span></label>
					    <textarea class="form-control unicase-form-control text-input" name="shippingaddress" require="required"><?php echo $row['shippingAddress'];?></textarea>
					  </div>



						<div class="form-group">
					    <label class="info-title" for="Trạng thái thanh toán ">Trạng thái vận chuyển <span>*</span></label>
			 <input type="text" class="form-control unicase-form-control text-input" id="shippingstate" name="shippingstate" value="<?php echo $row['shippingState'];?>" bắt buộc>
					  </div>
					  <div class="form-group">
					    <label class="info-title" for="Thành phố thanh toán">Thành phố vận chuyển <span>*</span></label>
					    <input type="text" class="form-control unicase-form-control text-input" id="shippingcity" name="shippingcity" Need="required" value="<?php echo $row['shippingCity' ];?>" >
					  </div>
 <div class="form-group">
					    <label class="info-title" for="Mã pin thanh toán">Mã pin vận chuyển <span>*</span></label>
					    <input type="text" class="form-control unicase-form-control text-input" id="shippingpincode" name="shippingpincode" known="required" value="<?php echo $row['shippingPincode' ];?>" >
					  </div>
		...
		<thead>
			<tr>
				...
				<div class="cart-grand-total">
						Tổng cộng<span class="inner-left-md"><?php echo $_SESSION['tp']="$totalprice". "$"; ?></span>
					</div>
				</th>
			</tr>
		</thead><!-- /thead -->
				...
				<tr>
					<td>
						<div class="cart-checkout-btn pull-right">
							<button type="submit" name="ordersubmit" class="btn btn-primary">ĐANG THANH TOÁN</button>
